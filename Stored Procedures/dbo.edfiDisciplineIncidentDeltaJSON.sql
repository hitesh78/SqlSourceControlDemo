...
-- Description:	discipline incidents
-- Rev. Notes:	changed how datasnapshot logic works
-- =============================================
CREATE       PROCEDURE [dbo].[edfiDisciplineIncidentDeltaJSON]
@SchoolYear int,
...
BEGIN
	...
	exec edfiDisciplineincidentJSON @SchoolYear, @CalendarStartDate, @CalendarEndDate, @NewCourseJSONstr output;

	insert into @NewCourseJSON
	Select
	JSON_VALUE(value, '$."incidentIdentifier"') as code,
	value as cpJSON
	From
	...
	OPENJSON(
		(
			...
			and
			edfiResource = 'DisciplineIncidents'
			and
	...
	Where JobID = @JobID
		and edfiResource = 'DisciplineIncidents';

	-- clears out the old snapshots is real JobID is passed
	IF EXISTS(
		Select 0
		FROM EdfiSubmissionStatus 
		Where edfiResource = 'DisciplineIncidents'
