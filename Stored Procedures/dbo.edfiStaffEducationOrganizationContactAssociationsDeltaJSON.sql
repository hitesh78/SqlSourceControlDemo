...
-- Description:	This returns the edfi Staff Ed Org Contact Associations JSON delta
-- Rev. Notes: enhanced primary key, changed how datasnapshot logic works
-- =============================================
CREATE   PROCEDURE [dbo].[edfiStaffEducationOrganizationContactAssociationsDeltaJSON]
@SchoolYear int,
...
BEGIN
	...
	Declare @NewSEOEAJSON table (staffUniqueId nvarchar(50) PRIMARY KEY, seoeaJSON nvarchar(1000));
	Declare @OldSEOEAJSON table (staffUniqueId nvarchar(50) PRIMARY KEY, seoeaJSON nvarchar(1000));

	exec edfiStaffEducationOrganizationContactAssociationsJSON @NewSEOEAJSONstr output;
	...
	OPENJSON(
		(
			...
			and
			edfiResource = 'StaffEducationOrganizationContactAssociations'
			and
	...
	IF EXISTS(
		Select 0
		FROM EdfiSubmissionStatus 
		Where edfiResource = 'StaffEducationOrganizationContactAssociations'
