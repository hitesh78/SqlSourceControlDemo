SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO


-- =============================================
-- Author:		Joey
-- Create date: 11/10/2021
-- Modified dt: 06/15/2022
-- Description:	This returns the edfi Section JSON
-- Parameters: Calendar Year 
-- =============================================
CREATE     PROCEDURE [dbo].[edfiSectionsJSON]
@SchoolYear int,
@CalendarStartDate date,
@CalendarEndDate date,
@SJSON nvarchar(max) output
AS
BEGIN
	SET NOCOUNT ON;

	Declare @SchoolID nvarchar(50) = (SELECT EdFiDOESchoolID FROM IntegrationSettings Where ID = 1);

	set @SJSON = (
		select 
			@SchoolID as [classPeriodReference.schoolId],
			b.PeriodSymbol as [classPeriodReference.name],
			MAX(b.CourseCode) as [courseOfferingReference.localCourseCode],
			@SchoolID as [courseOfferingReference.schoolId],
			@SchoolYear as [courseOfferingReference.schoolYear],
			'http://doe.in.gov/Descriptor/TermDescriptor.xml/' + b.[Sessions] as [courseOfferingReference.termDescriptor],
			@SchoolID as [locationReference.schoolId],
			MAX(b.[Location]) as [locationReference.classroomIdentificationCode],
			@SchoolID as [schoolReference.schoolId],
			MAX(b.Units) as [availableCredits],
			MAX(b.MediumOfInstruction) as [mediumOfInstructionType],
			1 as [sequenceOfCourse],
			b.uniqueSectionCode
		from (
			Select 
				a.PeriodSymbol,
				a.CourseCode,
				a.[Sessions],
				isnull(nullif(a.[Location], ''), 'Classroom') as [Location],
				a.Units,
				isnull(a.MediumOfInstruction, 'Face-to-face instruction') as [MediumOfInstruction],
				isnull(nullif(a.[Location], ''), 'Classroom') + a.CourseCode + CONVERT(nvarchar(20), a.TeacherID) as [uniqueSectionCode]
				From (
					Select
						CASE
							WHEN c.ScheduleType = 1
							THEN (
								Select PeriodSymbol 
								From [Periods]
								Where PeriodID = c.[Period])
							WHEN c.ScheduleType IN (2, 3)
							THEN (
								Select PeriodSymbol 
								From [Periods]
								Where PeriodID = Coalesce(
									nullif(PeriodOnMonday,0),
									nullif(PeriodOnTuesday,0),
									nullif(PeriodOnWednesday,0),
									nullif(PeriodOnThursday,0),
									nullif(PeriodOnFriday,0),
									nullif(PeriodOnSaturday,0),
									nullif(PeriodOnSunday,0),
									nullif(BPeriodOnMonday,0),
									nullif(BPeriodOnTuesday,0),
									nullif(BPeriodOnWednesday,0),
									nullif(BPeriodOnThursday,0),
									nullif(BPeriodOnFriday,0),
									nullif(BPeriodOnSaturday,0),
									nullif(BPeriodOnSunday,0), 0))
							ELSE ''
						END as [PeriodSymbol],
						c.CourseCode,
						ep.[Sessions],
						CASE
							WHEN c.ScheduleType = 1
							THEN c.[Location]
							WHEN c.ScheduleType IN (2, 3)
							THEN (
								Select [Location] 
								From Locations 
								Where LocationID = Coalesce(
									nullif(c.LocationOnMonday,0),
									nullif(c.LocationOnTuesday,0),
									nullif(c.LocationOnWednesday,0),
									nullif(c.LocationOnThursday,0),
									nullif(c.LocationOnFriday,0),
									nullif(c.LocationOnSaturday,0),
									nullif(c.LocationOnSunday,0),
									nullif(c.BLocationOnMonday,0),
									nullif(c.BLocationOnTuesday,0),
									nullif(c.BLocationOnWednesday,0),
									nullif(c.BLocationOnThursday,0),
									nullif(c.BLocationOnFriday,0),
									nullif(c.BLocationOnSaturday,0),
									nullif(c.BLocationOnSunday,0), 0))				
							ELSE ''
						END as [Location],
						c.Units,
						c.MediumOfInstruction,
						c.TeacherID
					From Classes c
						inner join Terms t
							on t.TermID = c.TermID
						inner join EdfiPeriods ep
							on ep.EdfiPeriodID = t.EdfiPeriodID
					Where t.StartDate >= @CalendarStartDate
						and t.EndDate <= @CalendarEndDate
						and c.ClassTypeID IN (1, 8) -- <> 5
						and ISNULL(NULLIF(c.CourseCode, ''), 'N/A') <> 'N/A' -- covers 3 cases: null, blank and 'N/A'
					UNION ALL
					-- secondary teachers
					Select
						CASE
							WHEN c.ScheduleType = 1
							THEN (
								Select PeriodSymbol 
								From [Periods]
								Where PeriodID = c.[Period])
							WHEN c.ScheduleType IN (2, 3)
							THEN (
								Select PeriodSymbol 
								From [Periods]
								Where PeriodID = Coalesce(
									nullif(PeriodOnMonday,0),
									nullif(PeriodOnTuesday,0),
									nullif(PeriodOnWednesday,0),
									nullif(PeriodOnThursday,0),
									nullif(PeriodOnFriday,0),
									nullif(PeriodOnSaturday,0),
									nullif(PeriodOnSunday,0),
									nullif(BPeriodOnMonday,0),
									nullif(BPeriodOnTuesday,0),
									nullif(BPeriodOnWednesday,0),
									nullif(BPeriodOnThursday,0),
									nullif(BPeriodOnFriday,0),
									nullif(BPeriodOnSaturday,0),
									nullif(BPeriodOnSunday,0), 0))
							ELSE ''
						END as [PeriodSymbol],
						c.CourseCode,
						ep.[Sessions],
						CASE
							WHEN c.ScheduleType = 1
							THEN c.[Location]
							WHEN c.ScheduleType IN (2, 3)
							THEN (
								Select [Location] 
								From Locations 
								Where LocationID = Coalesce(
									nullif(c.LocationOnMonday,0),
									nullif(c.LocationOnTuesday,0),
									nullif(c.LocationOnWednesday,0),
									nullif(c.LocationOnThursday,0),
									nullif(c.LocationOnFriday,0),
									nullif(c.LocationOnSaturday,0),
									nullif(c.LocationOnSunday,0),
									nullif(c.BLocationOnMonday,0),
									nullif(c.BLocationOnTuesday,0),
									nullif(c.BLocationOnWednesday,0),
									nullif(c.BLocationOnThursday,0),
									nullif(c.BLocationOnFriday,0),
									nullif(c.BLocationOnSaturday,0),
									nullif(c.BLocationOnSunday,0), 0))				
							ELSE ''
						END as [Location],
						c.Units,
						c.MediumOfInstruction,
						tc.TeacherID
					From TeachersClasses tc
						inner join Classes c
							on c.ClassID = tc.ClassID
						inner join Terms t
							on t.TermID = c.TermID
						inner join EdfiPeriods ep
							on ep.EdfiPeriodID = t.EdfiPeriodID
					Where t.StartDate >= @CalendarStartDate
						and t.EndDate <= @CalendarEndDate
						and c.ClassTypeID IN (1, 8) -- <> 5
						and ISNULL(NULLIF(c.CourseCode, ''), 'N/A') <> 'N/A' -- covers 3 cases: null, blank and 'N/A'
						and ISNULL(tc.TeacherRole, '') <> ''
			) a
		) b
		group by b.uniqueSectionCode, b.Sessions, b.PeriodSymbol
		FOR JSON PATH
	);

END
GO
