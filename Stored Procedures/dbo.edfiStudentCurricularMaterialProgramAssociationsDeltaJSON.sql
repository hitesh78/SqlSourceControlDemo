...
-- Description:	This returns the edfi Students Curricular Material program JSON 
-- Rev. Notes:	changed how datasnapshot logic works
-- =============================================
CREATE         PROCEDURE [dbo].[edfiStudentCurricularMaterialProgramAssociationsDeltaJSON]
@SchoolYear int,
...
BEGIN
	...
	Declare @NewSSAssnsJSON table (studentProgramKey nvarchar(200) PRIMARY KEY, ssAssnJSON nvarchar(2000));
	Declare @OldSSAssnsJSON table (studentProgramKey nvarchar(200) PRIMARY KEY, ssAssnJSON nvarchar(2000));

	exec edfiStudentCurricularMaterialProgramAssociationsJSON @SchoolYear, @CalendarStartDate, @CalendarEndDate, @NewSSAssnsJSONstr output;

	insert into @NewSSAssnsJSON
	Select
	JSON_VALUE(value, '$.studentReference."studentUniqueId"') + ':' + JSON_VALUE(value, '$.programReference."name"') + ':' + JSON_VALUE(value, '$.livesWithParentReference."parentUniqueId"') + ':' + JSON_VALUE(value, '$."beginDate"') as studentProgramKey,
	value as ssAssnJSON
	From
	...
	OPENJSON(
		(
			...
			and
			edfiResource = 'StudentCurricularMaterialProgramAssociations'
			and
	...
	IF EXISTS(
		Select 0
		FROM EdfiSubmissionStatus 
		Where edfiResource = 'StudentCurricularMaterialProgramAssociations'
