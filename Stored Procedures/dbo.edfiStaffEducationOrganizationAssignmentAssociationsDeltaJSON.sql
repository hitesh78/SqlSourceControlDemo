...
-- Description:	This returns the edfi StaffEducationOrganizationAssignmentAssociations JSON delta
-- Rev. Notes:	changed how datasnapshot logic works
-- =============================================
CREATE   PROCEDURE [dbo].[edfiStaffEducationOrganizationAssignmentAssociationsDeltaJSON]
@SchoolYear int,
...
BEGIN
	...
	Declare @NewSEOEAJSON table (staffUniqueId nvarchar(150) PRIMARY KEY, seoeaJSON nvarchar(1000));
	Declare @OldSEOEAJSON table (staffUniqueId nvarchar(150) PRIMARY KEY, seoeaJSON nvarchar(1000));

	exec edfiStaffEducationOrganizationAssignmentAssociationsJSON @NewSEOEAJSONstr output;
	...
	substring(JSON_VALUE(value, '$."staffClassificationDescriptor"'), PATINDEX('%.xml%', JSON_VALUE(value, '$."staffClassificationDescriptor"')) + 5, 100) as staffUniqueId,
	value as seoeaJSON
	From
	...
	OPENJSON(
		(
			...
			and
			edfiResource = 'StaffEducationOrganizationAssignmentAssociations'
			and
	...
	IF EXISTS(
		Select 0
		FROM EdfiSubmissionStatus 
		Where edfiResource = 'StaffEducationOrganizationAssignmentAssociations'
