...
SET ANSI_NULLS ON
GO
CREATE Procedure [dbo].[glRefreshLunchCharges] (@table_name nvarchar(50), @table_pk_id int = null, @error nvarchar(4000)=null OUTPUT)
as
begin
...
--	BEGIN TRANSACTION
	BEGIN TRY
		...
		begin
			...
			if @Status <> 'Open'
				RAISERROR ('You may only post/refresh lunch charges for an open period.',18, 1);
			...
			insert into Receivables (StudentID,Date,TransactionTypeID,Amount)
				select StudentID,Date,TransactionTypeID,Amount 
					from dbo.funcLunchCharges(@InvoicePeriodID)
