...
-- Author:		Legacy/Joey
-- Create date: Legacy
-- Modified dt: 11/07/2022
-- Description:	Gets Current Local Date
-- Rev. Notes:	removed end date +1
-- =============================================
CREATE   FUNCTION [dbo].[GLgetdatetime]()
RETURNS datetime
AS
BEGIN
	...
	Begin
		...
		if convert(date, @TimeInSpecifiedTimeZone) between @CurrentDSTStartDate and @CurrentDSTEndDate --DATEADD(dd, 1, @CurrentDSTEndDate)
		Begin
			Set @TimeInSpecifiedTimeZone =  DATEADD(hh,1,@TimeInSpecifiedTimeZone)
		End
	End

  return @TimeInSpecifiedTimeZone
