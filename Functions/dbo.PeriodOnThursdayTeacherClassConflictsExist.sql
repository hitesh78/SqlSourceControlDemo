...
-- =============================================
CREATE FUNCTION [dbo].[PeriodOnThursdayTeacherClassConflictsExist]
(
	@TermID int,
	@ClassID int,
	@TeacherID int,
		...
		(
			...
			Periods P2
				on	(C2.ScheduleType = 1 and C2.Period > 0 and C2.Period = P2.PeriodID)
					or
					(C2.ScheduleType > 1 and C2.PeriodOnThursday > 0 and C2.PeriodOnThursday = P2.PeriodID)
			Where
			C2.ClassID = @ClassID
			...
			on	
				...
				or
				(C2.ScheduleType = 2 and C2.PeriodOnThursday > 0 and C2.PeriodOnThursday = P2.PeriodID)
				or
				(C2.ScheduleType = 3 and C2.BPeriodOnThursday > 0 and C2.BPeriodOnThursday = P2.PeriodID)
			Where
			C2.ClassID = @ClassID
		) x
			inner join
