...
-- =============================================
CREATE FUNCTION [dbo].[PeriodOnTuesdayTeacherClassConflictsExist]
(
	@TermID int,
	@ClassID int,
	@TeacherID int,
		...
		(
			...
			Periods P2
				on	(C2.ScheduleType = 1 and C2.Period > 0 and C2.Period = P2.PeriodID)
					or
					(C2.ScheduleType > 1 and C2.PeriodOnTuesday > 0 and C2.PeriodOnTuesday = P2.PeriodID)
			Where
			C2.ClassID = @ClassID
			...
			on	
				...
				or
				(C2.ScheduleType = 2 and C2.PeriodOnTuesday > 0 and C2.PeriodOnTuesday = P2.PeriodID)
				or
				(C2.ScheduleType = 3 and C2.BPeriodOnTuesday > 0 and C2.BPeriodOnTuesday = P2.PeriodID)
			Where
			C2.ClassID = @ClassID
		) x
			inner join
