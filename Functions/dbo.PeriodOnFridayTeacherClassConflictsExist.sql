...
-- =============================================
CREATE FUNCTION [dbo].[PeriodOnFridayTeacherClassConflictsExist]
(
	@TermID int,
	@ClassID int,
	@TeacherID int,
		...
		(
			...
			Periods P2
				on	(C2.ScheduleType = 1 and C2.Period > 0 and C2.Period = P2.PeriodID)
					or
					(C2.ScheduleType > 1 and C2.PeriodOnFriday > 0 and C2.PeriodOnFriday = P2.PeriodID)
			Where
			C2.ClassID = @ClassID
			...
			on	
				...
				or
				(C2.ScheduleType = 2 and C2.PeriodOnFriday > 0 and C2.PeriodOnFriday = P2.PeriodID)
				or
				(C2.ScheduleType = 3 and C2.BPeriodOnFriday > 0 and C2.BPeriodOnFriday = P2.PeriodID)
			Where
			C2.ClassID = @ClassID
		) x
			inner join
